...
class DatabaseManager:
    ...
    def migrate_from_json(self, json_file_path: str) -> Tuple[bool, int]:
        ...
        migration_marker = os.path.join(os.path.dirname(self.db_path),
                                        f".json_formula_migration_{os.path.basename(json_file_path)}")
    ...
    def manual_vacuum(self):
        """Manually trigger VACUUM to reclaim database space."""
        try:
            logging.info("Running manual VACUUM to reclaim database space")
            self.connection.execute('VACUUM')
            logging.info("Manual database VACUUM completed")
            return True
        except Exception as e:
            logging.error(f"Manual VACUUM failed: {e}")
            return False
