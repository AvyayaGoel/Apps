...
class ChemLab(QMainWindow):
    def __init__(self):
        ...
        self.copy_reaction_btn = QPushButton("📋 Copy")
        ...
        # Ctrl+C shortcut for copy selected reaction
        self.copy_shortcut = QShortcut(QKeySequence("Ctrl+C"), self)
        self.copy_shortcut.activated.connect(self.copy_selected_reaction)
    ...
    def create_reactions_table(self, parent_layout):
        ...
        self.copy_reaction_btn.setFont(nav_font)
        self.copy_reaction_btn.clicked.connect(self.copy_selected_reaction)
        nav_layout.addWidget(self.copy_reaction_btn)
    ...
    def on_reaction_key_press(self, event):
        ...
        if not ChemLabParser.handle_key_press_event(self.search_entry, event):
    ...
    def update_filter_menu(self):
        ...
        try:
            ...
            self.filter_menu.addAction("Exothermic", lambda: self._safe_apply_filter("Exothermic"))
            self.filter_menu.addAction("Endothermic", lambda: self._safe_apply_filter("Endothermic"))
    ...
    def copy_selected_reaction(self):
        """Copy the selected reaction text to clipboard"""
        selected_row = self.reactions_table.currentRow()
        if selected_row >= 0:
            # Column 1 is reaction text
            ...
            if clipboard:
                clipboard.setText(reaction_text)
            self.toast.info("Reaction copied to clipboard!")
    ...
    def view_selected_reaction(self):
        ...
        if selected_row >= 0:
            ...
            # Fetch reaction conditions from database
            conditions = self.db.get_reaction_conditions(reaction_id)
            temperature = conditions.get('temperature')
            temperature_unit = conditions.get('temperature_unit')
            pressure = conditions.get('pressure')
            pressure_unit = conditions.get('pressure_unit')
    ...
    def _on_reaction_hover(self, row, column):
        ...
        try:
            ...
            # Format reaction conditions with units
            ...
            if temp or pressure or catalyst:
                ...
                if temp:
                    ...
                    conditions_parts.append(f"🌡️ {temp_str}")
                if pressure:
                    pressure_str = f"{pressure}{pressure_unit}" if pressure_unit else pressure
                    conditions_parts.append(f"💨 {pressure_str}")
                if catalyst:
                    conditions_parts.append(f"⚡ {catalyst}")
                conditions_info = "<br><b>Conditions:</b> " + " | ".join(conditions_parts)
            ...
            <table cellpadding="10" cellspacing="0" width="{tooltip_width}" style="background-color: #3C3C3C;">
                ...
                <tr>
                    <td width="{tooltip_width}" style="background-color: #3C3C3C; color: #dddddd; font-size: 12px; padding-top: 6px;">
                        <b style="color: #ffffff;">Type:</b> {reaction_type}{heat_info}{conditions_info}
                    </td>
                </tr>
            </table>
            """
    ...
    def apply_filter_and_pagination(self):
        ...
        elif self.current_type_filter == "Exothermic":
            filtered = [
                r for r in filtered
                if (r.get('heat_type') or '').lower() == 'exothermic'
            ]
        elif self.current_type_filter == "Endothermic":
            filtered = [
                r for r in filtered
                if (r.get('heat_type') or '').lower() == 'endothermic'
            ]
        ...
        # Apply search filter - searches reaction text, type, heat type, catalyst, and notes
        if search_text:
            filtered = [
                r for r in filtered
                if search_text in r['reaction_text'].lower() or
                   search_text in r.get('reaction_type', '').lower() or
                   search_text in (r.get('heat_type') or '').lower() or
                   search_text in (r.get('catalyst') or '').lower() or
                   search_text in (r.get('notes') or '').lower()
            ]

        # Also filter by tags if searching
        if search_text:
            # Remove # prefix if present for tag search
            tag_search = search_text.lstrip('#')
            matching_tag_reaction_ids = self.db.search_reactions_by_tag(tag_search)
            if matching_tag_reaction_ids:
                # Include reactions that match by tag (even if they don't match other fields)
                filtered = [r for r in filtered if r['id'] in matching_tag_reaction_ids] + \
                           [r for r in self.all_reactions if r['id'] in matching_tag_reaction_ids
                            and r['id'] not in [f['id'] for f in filtered]]
    ...
    def load_data(self):
        ...
        try:
            ...
            # Apply filters first to get filtered_reactions populated
            self._apply_filters_only()

            # If we have a target reaction, find which page it's on in the FILTERED list
    ...
    def _apply_filters_only(self):
        """Apply search and type filters to populate filtered_reactions without pagination"""
        search_text = self.search_entry.text().lower()
        ...
        # Apply type filter if not "All"
        if self.current_type_filter == "Favorites":
            filtered = [r for r in filtered if r.get('is_favorite', 0)]
        elif self.current_type_filter == "Exothermic":
            filtered = [r for r in filtered if (r.get('heat_type') or '').lower() == 'exothermic']
        elif self.current_type_filter == "Endothermic":
            filtered = [r for r in filtered if (r.get('heat_type') or '').lower() == 'endothermic']
        elif self.current_type_filter != "All":
    ...
    def _jump_to_reaction_page(self, reaction_id):
        """Calculate and set the page containing the given reaction ID in filtered reactions"""
        # Find the index of the reaction in filtered_reactions
        for idx, reaction in enumerate(self.filtered_reactions):
            if reaction['id'] == reaction_id:
                # Calculate page number (1-indexed)
                page = (idx // self.items_per_page) + 1
                self.current_page = page
                return
        # If not found in filtered list, go to page 1
        self.current_page = 1
    ...
    def create_view_overlay(self, reaction_id, reaction_text, reaction_type,
                            heat_value=None, heat_type=None,
                            temperature=None, temperature_unit=None,
                            pressure=None, pressure_unit=None, catalyst=None,
        ...
        # Display reaction conditions if available
        if temperature or pressure or catalyst:
            conditions_parts = []
            if temperature:
                temp_str = f"{temperature}{temperature_unit}" if temperature_unit else temperature
                conditions_parts.append(f"Temperature: {temp_str}")
            if pressure:
                pressure_str = f"{pressure}{pressure_unit}" if pressure_unit else pressure
            ...
            conditions_label = QLabel(f"<b>Conditions:</b> {conditions_text}")
            conditions_label.setFont(QFont(FONT, 11))
            conditions_label.setStyleSheet("color: #dddddd;")
            info_layout.addWidget(conditions_label, 2, 0, 1, 2)

        # Display reaction notes if available
        if notes:
            notes_label = QLabel(f"<b>📝 Notes:</b> {notes}")
            notes_label.setFont(QFont(FONT, 11))
            notes_label.setStyleSheet("color: #90ee90;")  # Light green for notes
        ...
        if tags:
            ...
            tags_label = QLabel(f"<b>🏷️ Tags:</b> {tags_text}")
            tags_label.setFont(QFont(FONT, 11))
            tags_label.setStyleSheet("color: #dda0dd;")  # Plum color for tags
            tags_label.setWordWrap(True)
            info_layout.addWidget(tags_label, 4, 0, 1, 2)
    ...
    def selection_btn_toggle(self, toggle: bool = False):
        self.copy_reaction_btn.setVisible(toggle)
        self.view_reaction_btn.setEnabled(toggle)
        self.edit_reaction_btn.setEnabled(toggle)
        self.delete_reaction_btn.setEnabled(toggle)
