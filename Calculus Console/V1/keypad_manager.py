...
from constants import BUTTON_1_EVENT, MAIN_SYMBOL_SETS, SUPER_SCRIPT_SYMBOLS, SUB_SCRIPT_SYMBOLS, SUB_SCRIPT, \
    SUPER_SCRIPT
...
class KeypadManager:
    ...
    def _create_main_layout(self, parent_frame):
        ...
        tb.Button(layout_frame, text=SUPER_SCRIPT, width=12, bootstyle=SECONDARY,
                  command=lambda: self._switch_layout("super"), takefocus=False).pack(side=LEFT, padx=1, fill=X,
                                                                                      expand=YES)
        tb.Button(layout_frame, text=SUB_SCRIPT, width=12, bootstyle=SECONDARY,
                  command=lambda: self._switch_layout("sub"), takefocus=False).pack(side=LEFT, padx=1, fill=X,
                                                                                    expand=YES)
        ...
        if self.window:
            ...
            if self.last_position:
                self.window.geometry(
                    f"{required_width}x{required_height}+{self.last_position[0]}+{self.last_position[1]}")
            else:
