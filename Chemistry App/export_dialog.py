...
class ExportDialog(QDialog):
    ...
    def _on_type_checkbox_changed(self, state):
    ...
    def _perform_export(self):
        ...
        try:
            ...
            export_methods = {
                ...
                "text": self._export_to_txt
            }

            export_method = export_methods.get(export_format, self._export_to_txt)
            export_method(file_path, filtered)

            QMessageBox.information(self, "Export Complete", f"Successfully exported reactions to {export_format.upper()}!")
            self.accept()
        except Exception as e:
            QMessageBox.critical(self, "Export Error", f"Failed to export: {str(e)}")

    def _export_to_html(self, file_path, reactions):
        """Export reactions to HTML format with index of links"""
        # Sort reactions by reaction type
        reactions.sort(key=lambda r: (r.get('reaction_type', 'Unknown'), r['reaction_text']))
...
"""
    ...
    def _export_to_csv(self, file_path, reactions):
        """Export reactions to CSV format"""
        with open(file_path, 'w', newline='', encoding='utf-8-sig') as csvfile:
            fieldnames = ['ID', 'Reaction', 'Type', 'Compound Count']
    ...
    def _export_to_json(self, file_path, reactions):
    ...
    def _export_to_markdown(self, file_path, reactions):
    ...
    def _export_to_txt(self, file_path, reactions):
