...
from PyQt6.QtGui import (
    ...
    QFont, QFontMetrics, QPainter, QShortcut
)
...
class FormulaDetailsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        subject_color = self.subject_colors.get(info[2].strip(), "#3B82F6")

        # ── Build sections ──
        layout.addWidget(self._build_header())
        layout.addWidget(self._build_formula_box(info[1]))
        if notes and notes.strip():
            layout.addWidget(self._build_notes_section(notes))
        layout.addWidget(self._build_meta_chips(info, subject_color))

        if tags:
            layout.addWidget(self._build_tags_section(tags))

        layout.addWidget(self._build_variables_section(variables))
        layout.addStretch()
    ...
    def _build_meta_chips(self, info, subject_color: str):
        frame = QFrame()
        ...
        meta_items = [
            ("SUBJECT", info[2]),
            ("TOPIC", info[3]),
            ("SUB-TOPIC", info[4] if info[4] != "_GENERAL_" else "General"),
        ]
...
class CalculusConsoleV2(QMainWindow):
            ...
            for f in formulas:
                ...
                self.master_data[fid] = {
                    "main_info": [
                        ...
                        f["field"],
                        f["topic"],
                        f.get("sub_topic", "_GENERAL_"),
                        f.get("notes", "")
                    ],
                    "variables": f.get("variables", []),
                    "tags": self.db_manager.get_formula_tags(fid)
                }
    ...
    def _add_table_row(self, db_id: int, data: dict):
        info = data["main_info"]
    ...
    @staticmethod
    def _matches_filters(info: List, search: str, f_subj: str, f_topic: str, f_sub: str,
                         tags: List[str] | None = None) -> bool:
        """Check if a formula matches all active filters including tags."""
        if search:
            hay = f"{info[0]} {info[1]} {info[2]} {info[3]} {info[4]}".lower()
    ...
    def _get_filtered_ids(self) -> List[int]:
        """Return list of formula IDs matching current filters."""
        ...
        filtered = []
        for db_id_raw, data in self.master_data.items():
            info: List = data["main_info"]
            tags = data.get("tags", [])
            if not self._matches_filters(info, search, f_subj, f_topic, f_sub, tags):
                continue
            db_id = int(db_id_raw)
            filtered.append(int(db_id))
    ...
    def _save_formula_result(self, result: dict, is_edit: bool):
        ...
        try:
            if is_edit and result["id"] is not None:
                ...
                if success:
                    self.db_manager.replace_formula_tags(
                        result["id"],
                        result.get("tags", [])
                    )
                    ...
                    self.master_data[result["id"]] = {
                        "main_info": [result["id"], result["formula"], result["field"],
                                      result["topic"], result["sub_topic"], result["notes"]],
                ...
                if new_id:
                    self.db_manager.add_formula_tags(
                        new_id,
                        result.get("tags", [])
                    )
