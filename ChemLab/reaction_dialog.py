...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QWidget, QFrame,
...
from constants import (
    ...
    ADD_REACTION_BTN_TEXT, ICON_PATH, SUBSCRIPT_MAP
)
...
class SaveReactionWorker(QThread):
    ...
    def __init__(self, db, reaction, reaction_type, heat_value=None, heat_type=None,
                 temperature=None, temperature_unit=None, pressure=None, pressure_unit=None,
                 catalyst=None, notes=None, tags=None, current_reaction_id=None, compounds_data=None):
        super().__init__()
        self.db = db
    ...
    def run(self):
        try:
            ...
            # Save reaction to database with conditions
            if self.current_reaction_id:
                ...
                self.db.update_reaction(reaction_id, self.reaction, self.reaction_type,
                                        self.heat_value, self.heat_type,
                                        self.temperature, self.temperature_unit,
                                        self.pressure, self.pressure_unit, self.catalyst,
                                        self.notes)
            else:
            ...
            # Save tags
            self.progress.emit("Saving tags...")
            # Delete existing tags first if updating
            if self.current_reaction_id:
                self.db.delete_tags_for_reaction(reaction_id)
            # Add each tag
            for tag in self.tags:
                self.db.add_tag_to_reaction(reaction_id, tag)
    ...
    def run(self):
        try:
            ...
            for compound in compounds:
                ...
                # Normalize formula: convert subscripts to regular numbers for learner lookup
                # Learner stores formulas in ASCII form (e.g., "Al(OH)3" not "Al(OH)₃")
                normalized_formula = clean_formula.translate(SUBSCRIPT_MAP)

                # Get suggestions from learner
                suggested_name = self.compound_learner.get_name(normalized_formula) or ''
                suggested_color = self.compound_learner.get_color(normalized_formula, detected_state_abbr)
...
class ReactionDialog(QDialog):
    ...
    def __init__(self, db, reaction_id=None, parent=None):
        ...
        self.temp_unit_cb = QComboBox()
        self.pressure_entry = QLineEdit()
        self.pressure_unit_cb = QComboBox()
        self.catalyst_entry = QLineEdit()
        ...
        self._loading_edit_data = False  # Flag to block preview during edit load

        # Draft recovery setup
        self.draft_file = self._get_draft_file_path()
        self._draft_timer = QTimer()
        self._draft_timer.timeout.connect(self._save_draft)
        self._draft_timer.start(30000)  # Auto-save every 30 seconds
        ...
        # Load reaction data if editing (clear any existing draft first)
        if self.current_reaction_id:
            self._clear_draft()  # Clear draft from any previous new reaction
    ...
    def load_reaction_data(self, reaction_id):
        ...
        self._loading_edit_data = True  # Block preview updates
        try:
            ...
            # Set reaction text (show original with coefficients for visibility)
            ...
            temperature_unit = reaction.get('temperature_unit', '')
            ...
            if temperature:
                self.temp_entry.setText(str(temperature))
            if temperature_unit:
                self.temp_unit_cb.setCurrentText(str(temperature_unit))
            if pressure:
                self.pressure_entry.setText(str(pressure))
            if pressure_unit:
                self.pressure_unit_cb.setCurrentText(str(pressure_unit))
            if catalyst:
                self.catalyst_entry.setText(str(catalyst))
            if notes:
                self.notes_entry.setText(str(notes))

            # Load tags from database
            tags = self.db.get_tags_for_reaction(reaction_id)
            ...
            # Build map with normalized formulas (ASCII) for matching
            db_compound_map = {ChemLabParser.normalize_formula(c['formula']): c for c in db_compounds}

            # Build formatted compounds list
            formatted_compounds = []
            for comp in compounds:
                # Normalize formula for DB lookup (converts Unicode subscripts to ASCII)
                lookup_formula = ChemLabParser.normalize_formula(comp['formula'])
                db_comp = db_compound_map.get(lookup_formula, {})
            ...
            # Stop any pending preview updates
            self.preview_timer.stop()

            # Clear tracking dicts before loading DB data to ensure clean slate
            self._init_tracking_dicts()
            self.compounds_table.clearContents()

            # Update compounds table with force-load (for edit mode)
            self._load_table_for_edit(formatted_compounds)

        except Exception as e:
            logging.error(f"Failed to load reaction {reaction_id}: {e}")
            QMessageBox.warning(self, "Load Error", f"Failed to load reaction: {e}")
        finally:
            self._loading_edit_data = False  # Re-enable preview updates
    ...
    def init_ui(self):
        ...
        # Set validator to only allow valid chemical reaction characters
        # Allows: letters, numbers, brackets, +, -, =, <, >, Unicode arrows, space
        # No dots (hydrates handled separately), no special condition/heat symbols
        chem_validator = QRegularExpressionValidator(QRegularExpression(
            r"[a-zA-Z0-9₀-₉\(\)\[\]+\-=<>→←⇌⇋↔↑↓\s]*"
        ))
        self.reaction_entry.setValidator(chem_validator)
        ...
        # Reaction conditions (optional) - temperature, pressure, catalyst
        conditions_layout = QHBoxLayout()

        # Temperature input with unit selector
        self.temp_entry.setPlaceholderText("")
        self.temp_entry.setValidator(QDoubleValidator(0.0, 9999.0, 2))
        self.temp_unit_cb.addItems(['', '°C', 'K', '°F'])
        self.temp_unit_cb.setEditable(False)
        self.temp_unit_cb.setFixedWidth(60)
        conditions_layout.addWidget(QLabel("🌡️ Temp:"))
        temp_layout = QHBoxLayout()
        temp_layout.addWidget(self.temp_entry, 1)
        temp_layout.addWidget(self.temp_unit_cb)
        conditions_layout.addLayout(temp_layout, 1)

        # Pressure input with unit selector
        self.pressure_entry.setPlaceholderText("")
        self.pressure_entry.setValidator(QDoubleValidator(0.0, 999999.0, 4))
        self.pressure_unit_cb.addItems(['', 'atm', 'kPa', 'bar', 'mmHg', 'Torr'])
        self.pressure_unit_cb.setEditable(False)
        self.pressure_unit_cb.setFixedWidth(70)
        conditions_layout.addWidget(QLabel("💨 Pressure:"))
        pressure_layout = QHBoxLayout()
        pressure_layout.addWidget(self.pressure_entry, 1)
        pressure_layout.addWidget(self.pressure_unit_cb)
        conditions_layout.addLayout(pressure_layout, 1)

        # Catalyst input
        self.catalyst_entry.setPlaceholderText("Catalyst (e.g., Pt, MnO₂)")
        conditions_layout.addWidget(QLabel("⚡ Catalyst:"))
        conditions_layout.addWidget(self.catalyst_entry, 1)

        reaction_layout.addLayout(conditions_layout)

        # Reaction notes (full width)
        notes_layout = QHBoxLayout()
        self.notes_entry.setPlaceholderText("Reaction notes (e.g., Lab experiment #3, produces toxic fumes)")
        notes_layout.addWidget(QLabel("📝 Notes:"))
        notes_layout.addWidget(self.notes_entry, 1)
        reaction_layout.addLayout(notes_layout)

        # Tags section (full width)
        tags_layout = QVBoxLayout()
        tags_layout.setSpacing(5)

        # Tag input row with label, entry, and + button
        ...
        tag_input_layout.addWidget(QLabel("🏷️ Tags:"))
        tag_input_layout.addWidget(self.tag_entry, 1)
        tag_input_layout.addWidget(self.add_tag_btn)
        tags_layout.addLayout(tag_input_layout)

        # Container for tag chips (added dynamically)
        ...
        reaction_layout.addLayout(tags_layout)
        ...
        # Load draft if available (only in add mode, not edit mode)
    ...
    def clear_form(self):
        ...
        self.temp_unit_cb.setCurrentText("")
        self.pressure_entry.clear()
        self.pressure_unit_cb.setCurrentText("")
        self.catalyst_entry.clear()
    ...
    def on_add_tag_clicked(self):
        ...
        if len(self.current_tags) >= self.MAX_TAGS:
            return
        tag = self.tag_entry.text().strip().lower()
        if not tag:
            return
        ...
        self.tag_entry.clear()
        self._update_tag_controls_state()
    ...
    def _add_tag_chip(self, tag: str):
        ...
        tag_label.setStyleSheet("color: white; font-weight: bold;")
    ...
    def _remove_tag(self, tag: str, chip_widget):
        """Remove a tag from the list and UI."""
        if tag in self.current_tags:
    ...
    def _clear_all_tags(self):
        """Clear all tag chips from the UI."""
    ...
    def _load_tags(self, tags: list):
        """Load tags into the UI."""
        self._clear_all_tags()
        for tag in tags[:self.MAX_TAGS]:  # Only load up to max
            self.current_tags.append(tag)
            self._add_tag_chip(tag)
        self._update_tag_controls_state()

    def _update_tag_controls_state(self):
        """Enable/disable tag entry and add button based on tag count."""
        at_limit = len(self.current_tags) >= self.MAX_TAGS
        self.tag_entry.setEnabled(not at_limit)
        self.add_tag_btn.setEnabled(not at_limit)
        if at_limit:
            self.tag_entry.setPlaceholderText(f"Max {self.MAX_TAGS} tags reached")
        else:
            self.tag_entry.setPlaceholderText("Enter tag (e.g., organic, exam-chapter-5)")
    ...
    def handle_reaction_saved(self, reaction, reaction_type, heat_value, heat_type,
                              temperature, pressure, catalyst, compounds_data, reaction_id):
    ...
    def on_reaction_key_press(self, event):
        ...
        if not ChemLabParser.handle_key_press_event(self.reaction_entry, event):
            # Event not handled by shared utility, pass to default handler
    ...
    def on_reaction_changed(self):
        ...
        # Skip if loading edit data to prevent overwriting DB values
        if not self._loading_edit_data:
            self.preview_timer.stop()
    ...
    def update_preview_tables(self):
        ...
        # Skip preview during edit load to prevent overwriting DB values
        if self._loading_edit_data:
            return
    ...
    def _on_preview_finished(self, elements_data, compounds_list):
        ...
        # Skip if we started loading edit data while preview was running
        if self._loading_edit_data:
            return
        # Update elements table
    ...
    def _load_table_for_edit(self, db_compounds):
        """Load compounds table for edit mode - force all DB data and track as suggested."""
        self.compounds_table.blockSignals(True)
        try:
            self.compounds_table.setRowCount(len(db_compounds))

            for row, compound in enumerate(db_compounds):
                formula = compound['formula']
                comp_type = compound['type']
                name = compound.get('name', '')
                color = compound.get('color', '')
                state = compound.get('state', '')
                notes = compound.get('notes', '')

                # Formula (column 0) - not editable
                self._get_or_create_item(row, 0, ChemLabParser.display_formula(formula), editable=False)

                # Type (column 1) - not editable
                self._get_or_create_item(row, 1, comp_type, editable=False)

                # Name (column 2) - editable, force set from DB and track as suggested
                name_item = QTableWidgetItem(name)
                self.compounds_table.setItem(row, 2, name_item)
                self._last_suggested_names[row] = name
                self._last_formulas[row] = formula

                # Color (column 3) - editable, force set from DB and track as suggested
                color_item = QTableWidgetItem(color)
                self.compounds_table.setItem(row, 3, color_item)
                self._last_suggested_colors[row] = color

                # State (column 4) - not editable
                display_state = STATE_NAMES.get(state, state)
                self._get_or_create_item(row, 4, display_state, editable=False)
                self._last_state_abbrs[row] = state

                # Notes (column 5) - editable, force set from DB
                notes_item = QTableWidgetItem(notes)
                self.compounds_table.setItem(row, 5, notes_item)
        finally:
            self.compounds_table.blockSignals(False)
    ...
    def _get_suggested_name(self, formula):
        ...
        # Normalize formula: convert subscripts to regular numbers for lookup
        # Learner stores formulas in ASCII form (e.g., "Al(OH)3" not "Al(OH)₃")
        normalized_formula = formula.translate(SUBSCRIPT_MAP)
        name = self.compound_learner.get_name(normalized_formula)
        return name
    ...
    def _get_suggested_color(self, formula, state):
        ...
        # Normalize formula: convert subscripts to regular numbers for lookup
        normalized_formula = formula.translate(SUBSCRIPT_MAP)
        color = self.compound_learner.get_color(normalized_formula, state)
        return color
    ...
    def add_reaction(self):
        ...
        # Get reaction conditions (optional)
        temperature = self.temp_entry.text().strip() or None
        temperature_unit = self.temp_unit_cb.currentText() if temperature else None
        pressure = self.pressure_entry.text().strip() or None
        pressure_unit = self.pressure_unit_cb.currentText() if pressure else None
        catalyst = self.catalyst_entry.text().strip() or None
        notes = self.notes_entry.text().strip() or None
        ...
        self._worker = SaveReactionWorker(
            self.db, reaction, reaction_type, heat_value, heat_type,
            temperature, temperature_unit, pressure, pressure_unit, catalyst,
            notes, self.current_tags, self.current_reaction_id, compounds_data
        )
    ...
    def _on_balanced_reaction(self, balanced):
        ...
        # Update the entry with balanced version (show coefficients for visibility)
    ...
    def _on_save_finished(self, success, error_message, reaction_id):
        ...
        if success:
            # Clear draft file after successful save
            self._clear_draft()
    ...
    def closeEvent(self, event):
        """Handle dialog close - warn about unsaved changes and save draft (only for new reactions)"""
        try:
        ...
        # Skip draft handling when editing an existing reaction
        if self.current_reaction_id:
        ...
        # For new reactions: Check for unsaved changes (reaction with arrow present)
        reaction_text = self.reaction_entry.text().strip()
        has_unsaved_changes = any(arrow in reaction_text for arrow in ARROWS)

        if has_unsaved_changes:
            # Warn user about unsaved changes
            reply = QMessageBox.question(
                self, 'Unsaved Changes',
                'You have unsaved reaction data.\n\nSave draft and close?\n(Click No to close without saving draft)',
    ...
    def _get_draft_file_path():
        """Get path to draft file in local app data"""
        local_appdata = os.environ.get('LOCALAPPDATA')
        if local_appdata:
            data_dir = os.path.join(local_appdata, 'ChemLab Data')
        else:
            data_dir = os.path.join(os.path.expanduser('~'), 'ChemLab Data')
        os.makedirs(data_dir, exist_ok=True)
        return os.path.join(data_dir, 'reaction_draft.json')

    def _save_draft(self):
        """Auto-save current reaction data to draft file (only for new reactions)"""
        try:
            # Don't save draft when editing an existing reaction
            if self.current_reaction_id:
                return
            ...
            draft_data = {
                ...
                'heat_value': self.heat_value_entry.text().strip(),
                'heat_type': self.heat_type_cb.currentText(),
                'temperature': self.temp_entry.text().strip(),
                'temperature_unit': self.temp_unit_cb.currentText(),
                'pressure': self.pressure_entry.text().strip(),
                'pressure_unit': self.pressure_unit_cb.currentText(),
                'catalyst': self.catalyst_entry.text().strip(),
                'current_reaction_id': self.current_reaction_id,
                'timestamp': None
            }
            ...
            logging.error(f"[Draft] Failed to save draft: {e}")

    def _load_draft(self):
        """Load draft data from file if it exists (only for new reactions, not edits)"""
        try:
            ...
            # Skip loading if this is an edit draft (contains reaction_id)
            # Drafts should only be for new reactions being composed
            if draft_data.get('current_reaction_id'):
                self._clear_draft()
                return False

            self.reaction_entry.setText(draft_data.get('reaction_text', ''))

            reaction_type = draft_data.get('reaction_type', '')
            if reaction_type:
                self.reaction_type_cb.setCurrentText(reaction_type)

            self.heat_value_entry.setText(draft_data.get('heat_value', ''))

            heat_type = draft_data.get('heat_type', '')
            if heat_type:
                self.heat_type_cb.setCurrentText(heat_type)

            self.temp_entry.setText(draft_data.get('temperature', ''))
            temp_unit = draft_data.get('temperature_unit', '')
            if temp_unit:
                self.temp_unit_cb.setCurrentText(temp_unit)
            self.pressure_entry.setText(draft_data.get('pressure', ''))
            pressure_unit = draft_data.get('pressure_unit', '')
            if pressure_unit:
                self.pressure_unit_cb.setCurrentText(pressure_unit)
            self.catalyst_entry.setText(draft_data.get('catalyst', ''))

            reaction_id = draft_data.get('current_reaction_id')
            if reaction_id:
                self.current_reaction_id = reaction_id
                self.add_reaction_btn.setText("Update Reaction")
                self.setWindowTitle(f"Edit Reaction #{reaction_id}")

            self._clear_draft()
            return True

        except Exception as e:
            logging.error(f"[Draft] Failed to load draft: {e}")
            return False
    ...
    def _clear_draft(self):
        ...
        try:
            ...
            logging.error(f"[Draft] Failed to clear draft: {e}")
