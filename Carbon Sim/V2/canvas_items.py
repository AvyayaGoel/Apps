...
This is the visual heart of the app.
Atoms: Jmol-colored spheres with radial gradients (3D look),
       auto-contrasting element symbols, formal charges.
Bonds: Single / double / triple parallel lines with proper offsets.
...
class AtomItem(QGraphicsItem):
    ...
    Renders as a shaded sphere (radial gradient) with the element
    symbol centered.  Formal charge appears above if non-zero.
    Selection adds a golden halo.
    """
...
class BondItem(QGraphicsItem):
    ...
    Renders as parallel line(s):
        S → 1 line
        D → 2 lines (offset ±spacing/2)
        T → 3 lines (offset 0, ±spacing)
    """
    ...
    def boundingRect(self) -> QRectF:
        try:
            ...
            if not a1 or not a2:
                logger.warning(f'BondItem.boundingRect: missing atom(s) for bond {self.bond.a1}-{self.bond.a2}')
                return QRectF()
            order = BOND_ORDER_VALUE.get(self.bond.type, 1)
            # Visual line count differs from chemical bond order for aromatic
            # (drawn as 2 parallel lines — one solid, one inner dashed — even
            # though its order is 1.5) and dative (drawn as 1 line + arrowhead,
            # order 1). Use line count for layout margin, not raw order.
            line_count = 2 if self.bond.type == 'A' else 1 if self.bond.type == 'DA' else max(int(order), 1)
            spacing = 6 * self.zoom
    ...
    def paint(self, painter, option, widget=None):
        try:
            ...
            a2 = self.mol.get_atom(self.bond.a2)
            if not a1 or not a2:
                logger.warning('BondItem.paint: missing atom(s), aborting paint')
                return
            ax, ay = (a1.x, a1.y)
            bx, by = (a2.x, a2.y)
            dx, dy = (bx - ax, by - ay)
            d = math.hypot(dx, dy)
            if d == 0:
                logger.warning(f'BondItem.paint: zero-length bond {self.bond.a1}-{self.bond.a2}, aborting')
                return
            nx, ny = (-dy / d, dx / d)
            ux, uy = (dx / d, dy / d)
            ...
            if self.bond.type == 'A':
                # Aromatic: one solid full-length line plus a shorter, inset,
                # dashed inner line — the standard chemistry-drawing
                # convention for a delocalized (order 1.5) bond.
            ...
            if self.bond.type == 'DA':
                # Dative/coordinate bond: single line ending in an arrowhead
                # pointing from donor (a1) to acceptor (a2) — e.g. N->B.
                # Pull the tip back to the acceptor atom's visual edge (atoms
                # render above bonds, zValue 10 vs 5) so the arrowhead is
                # actually visible instead of hidden under the atom sphere.
            ...
            # Single / Double / Triple: parallel line(s), as before.
            order = BOND_ORDER_VALUE.get(self.bond.type, 1)
    ...
    def hit_test(self, px: float, py: float, threshold: float = 8) -> bool:
        """Point-to-segment distance for any of the parallel bond lines."""
        a1 = self.mol.get_atom(self.bond.a1)
        a2 = self.mol.get_atom(self.bond.a2)
        if not a1 or not a2:
            logger.warning(f'BondItem.hit_test: missing atom(s) for bond {self.bond.a1}-{self.bond.a2}')
            return False
        ax, ay = (a1.x, a1.y)
        bx, by = (a2.x, a2.y)
        dx, dy = (bx - ax, by - ay)
        ...
        for o in range(line_count):
            ...
            if vlen2 == 0:
                ...
                if dist <= threshold:
                    logger.info(f'BondItem.hit_test HIT on line {o} (zero-length endpoint)')
                    return True
                continue
            ...
            if dist <= threshold:
                logger.info(f'BondItem.hit_test HIT on line {o}')
                return True
        return False
