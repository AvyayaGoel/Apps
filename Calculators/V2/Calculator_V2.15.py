...
FACTORIAL_LIMIT = 18  # exact factorial cutoff


# -------------------------------------------------
# NORMALIZATION (KEEP — already correct)
# -------------------------------------------------
...
def prod_range(a: int, b: int) -> int:
    if a > b:
        return 1
    result = 1
...
# -------------------------------------------------
# FACTORIAL SIMPLIFICATION (KEEP, but ORDER MATTERS)
# -------------------------------------------------

def simplify_factorial_fractions(expr: str) -> str:
    """
    Cancels expressions like:
    100!/99! → 100
    99!/100! → 1/100
    """
    frac_pattern = re.compile(r'(\d+)!/(\d+)!')

    while True:
        m = frac_pattern.search(expr)
        if not m:
            break
        ...
        if a == b:
            ...
            repl = f"({prod_range(b + 1, a)})"
        else:
            repl = f"(1/{prod_range(a + 1, b)})"
...
def cancel_factorial_identities(expr: str) -> str:
    """
    Finds chains like 100!*100!/99!/99! and cancels them symbolically.
    """

    # Pattern to find a block of multiplications/divisions of factorials
    # e.g., 20!/10!*9!
    def simplify_group(match):
        term = match.group(0)
        # Split into parts like ['20!', '/', '10!', '*', '9!']
        parts = re.split(r'([*/])', term)
        nums, dens, current_op = [], [], '*'
        ...
        # Identity: Cancel n! / m! -> Product(m+1, n)
        nums.sort(reverse=True)
        dens.sort(reverse=True)

        results = []
        for d in dens:
            # Find best numerator to cancel with (n >= d)
            found = False
            for i, n in enumerate(nums):
                if n >= d and n - d < 5000:  # Identity range
                    if n == d:
                        results.append(1)
                    else:
                        prod = 1.0
                        for v in range(d + 1, n + 1): prod *= v
                        results.append(prod)
                    nums.pop(i)
                    found = True
                    break
            if not found: results.append(f"/(math.factorial({d}))")  # Keep it if no match

        # Rebuild the simplified term
        final_parts = [str(x) for x in nums] + [str(x) for x in results]
        # Filter out division strings and join
        muls = "*".join([x for x in final_parts if not x.startswith('/')])
        divs = "".join([x for x in final_parts if x.startswith('/')])
        return f"({muls}{divs})"

    # Match any sequence of numbers with factorials separated by * or /
    expr = re.sub(r'\d+!(?:[*/]\d+!)+', simplify_group, expr)
    # Single factorials handled by normal math.factorial
    expr = re.sub(r'(\d+)!', r'math.factorial(\1)', expr)
    return expr


# -------------------------------------------------
# SCIENTIFIC FACTORIAL (KEEP)
# -------------------------------------------------

def factorial_sci(n: int) -> str:
    if n < 0:
    ...
    # SCINUM is NEVER evaluated directly
    return f"SCINUM({mantissa:.10f},{exponent})"


def simplify_standalone_factorials(expr: str) -> str:
    """
    Only resolves factorials that are NOT part of a ratio.
    """
...
# -------------------------------------------------
# PERCENT LOGIC (UPGRADED, IDENTITY-BASED)
# -------------------------------------------------

def rewrite_postfix_percent(expr: str) -> str:
    """
    Identity:
    x% → x/100
    """
...
def rewrite_consumer_percent_with_mul(expr: str) -> str:
    """
    Handles:
    A ± B% * C  →  A ± (A * B / 100 * C)
    """
    ...
    while True:
        ...
        base, op, pct, mul = m.groups()
        repl = f"({base}{op}({base}*{pct}/100*{mul}))"
        expr = expr[:m.start()] + repl + expr[m.end():]

    return expr


def rewrite_consumer_percent(expr: str) -> str:
    """
    Windows-style consumer percent.
    Applies A ± B% → A ± (A * B / 100)
    but ONLY when A is a fully-resolved base.
    """
    pat = re.compile(
        r'(\([^()]*\)|\d+(?:\.\d+)?)([+-])\((\d+(?:\.\d+)?)/100\)'
    )
    ...
    while True:
        ...
        base, op, pct = m.groups()

        # 🚨 GUARD: do not rebind if base already contains percent logic
        if "/100" in base:
            break

        repl = f"({base}{op}({base}*{pct}/100))"
        expr = expr[:m.start()] + repl + expr[m.end():]
...
# -------------------------------------------------
# FINAL EVALUATION (KEEP)
# -------------------------------------------------

def final_eval(expr: str):
    """
    Evaluates expression safely, resolving SCINUM only at the end.
    """

    def resolve_scinum(match):
        return f"{match.group(1)}e{match.group(2)}"

    processed_expr = re.sub(
        r'SCINUM\(\s*([\d.]+)\s*,\s*(\d+)\s*\)',
        resolve_scinum,
        expr
    )

    try:
        val = eval(processed_expr, {"__builtins__": {}})
...
def rewrite_percent_logic(expr: str) -> str:
    """
    Handles Percentages with BODMAS awareness.
    """
    # 1. STANDALONE/BODMAS PRIORITY
    # If a % is followed by * or / (e.g., 50% * 2), it is NOT a consumer percent.
    # We turn it into a fraction immediately so BODMAS handles it correctly.
    expr = re.sub(r'(\d+(?:\.\d+)?)%(?=[*/^])', r'(\1/100)', expr)

    # 2. CONSUMER MULTIPLIER (Windows Style: A ± B% * C)
    # This only triggers if there is a base AND no higher priority conflicts.
    # We use a pattern that ensures the % is part of a + or - operation.
    pat_mul = re.compile(r'(\d+(?:\.\d+)?)([+-])(\d+(?:\.\d+)?)%\*(\d+(?:\.\d+)?)')
    while True:
        m = pat_mul.search(expr)
        if not m: break
        base, op, pct, mul = m.groups()
        # Rule: A ± (A * pct/100 * mul)
        repl = f"({base}{op}({base}*{pct}/100*{mul}))"
        expr = expr[:m.start()] + repl + expr[m.end():]

    # 3. STANDARD CONSUMER (A ± B%)
    pat_cons = re.compile(r'(\d+(?:\.\d+)?)([+-])(\d+(?:\.\d+)?)%')
    while True:
        m = pat_cons.search(expr)
        if not m: break
        base, op, pct = m.groups()
        repl = f"({base}{op}({base}*{pct}/100))"
        expr = expr[:m.start()] + repl + expr[m.end():]
    ...
    # 4. FINAL CLEANUP
    # Any leftover % (like 888/23%) just becomes /100
    expr = re.sub(r'(\d+(?:\.\d+)?)%', r'(\1/100)', expr)
...
# -------------------------------------------------
# MASTER SIMPLIFIER (UPDATED ORDER, CRITICAL)
# -------------------------------------------------

def fully_simplify(expr: str) -> str:
    """
    Identity-driven rewrite loop.
    Simulates Windows Calculator semantics
    but evaluates ONLY after full expression is written.
    """
    prev = None
    ...
    while prev != expr:
        ...
        # -------------------------------------------------
        # 1. Factorial cancellation FIRST (critical)
        # -------------------------------------------------
        expr = simplify_factorial_fractions(expr)
        expr = cancel_factorial_identities(expr)

        # -------------------------------------------------
        # 2. Resolve standalone factorials ONLY
        # (never expand ratios)
        # -------------------------------------------------
        expr = simplify_standalone_factorials(expr)

        # -------------------------------------------------
        # 3. Percent logic (Windows-style, expression-based)
        # -------------------------------------------------
        expr = rewrite_percent_logic(expr)
        # 3a. Convert postfix percent to fraction
        # 50% → (50/100)
        expr = re.sub(
            r'(\d+(?:\.\d+)?)%',
            r'(\1/100)',
        ...
        # 3b. Consumer percent binding
        # A + (B/100) → A + (A*B/100)
        # A - (B/100) → A - (A*B/100)
        expr = rewrite_consumer_percent_with_mul(expr)
        expr = rewrite_consumer_percent(expr)
...
class Calculator:
    ...
    def __init__(self, window):
        self.root = window
        self.root.title("Calculator_V2.15")
    ...
    def setup_converter(self, frame, v_in, v_out, units, cmd, clr_cmd, back_cmd, num_cmd):
        ...
        for text, col, row in buttons:
            btn = tb.Button(
                ...
                num_cmd(x) if x not in ('CE', '←')
                else clr_cmd() if x == 'CE'
                else back_cmd()
            )
    ...
    def setup_length_converter_tab(self):
        ...
        self.length_input_text.trace_add("write", lambda *a: self.convert_length())
    ...
    def setup_area_converter_tab(self):
        ...
        self.area_input_text.trace_add("write", lambda *a: self.convert_area())
    ...
    def setup_temp_converter_tab(self):
        ...
        self.temp_input_text.trace_add("write", lambda *a: self.convert_temp())
